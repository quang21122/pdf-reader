...
export function getOCRStats(ocrResults: OCRResult[]) {
  ...
  const lowConfidencePages = ocrResults.filter(result => result.confidence < 70)
  const highConfidencePages = ocrResults.filter(result => result.confidence >= 90)
  ...
  return {
    ...
    confidenceDistribution: {
      ...
      good: ocrResults.filter(r => r.confidence >= 85 && r.confidence < 95).length,
      fair: ocrResults.filter(r => r.confidence >= 70 && r.confidence < 85).length,
