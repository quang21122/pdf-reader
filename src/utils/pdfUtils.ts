 ...
 */
export async function pdfToImages(pdfUrl: string, scale: number = 2): Promise<string[]> {
  try {
    const pdf = await pdfjsLib.getDocument(pdfUrl).promise
    const images: string[] = []
 ...
 */
export async function searchInPDF(pdfUrl: string, searchTerm: string): Promise<Array<{
  pageNumber: number
