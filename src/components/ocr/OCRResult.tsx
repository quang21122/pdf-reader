  ...
  if (results.length === 0) {
    return (
      <Box className="p-4 text-center">
        <Typography variant="body2" color="text.secondary">
          No OCR results available. Upload a PDF and run OCR to see extracted text.
        </Typography>
      </Box>
    )
  }
    ...
    <Box className="p-4">
      <Box className="mb-4">
        ...
        <Box className="flex items-center gap-2 mb-2">
          ...
          />
          <Chip
            label={`${getConfidenceLabel(totalConfidence)} (${Math.round(totalConfidence)}%)`}
            size="small"
            color={getConfidenceColor(totalConfidence) as any}
          />
        </Box>
      </Box>
                ...
                {result.words && result.words.length > 0 && (
                  <Box className="mt-3">
                    <Typography variant="caption" color="text.secondary" className="mb-2 block">
                      Word-level confidence (showing words with confidence &lt; 80%):
                    </Typography>
                    <Box className="flex flex-wrap gap-1">
                      {result.words
                        ...
                        .map((word, index) => (
                          <Chip
                            key={index}
                            label={`${word.text} (${Math.round(word.confidence)}%)`}
                            size="small"
                            variant="outlined"
                            color="warning"
                          />
