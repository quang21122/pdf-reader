...
export function usePDFValidation() {
  ...
  const [lastValidationResult, setLastValidationResult] = useState<PDFValidationResult | null>(null);
  ...
  const checkPDFAccessibility = useCallback(async (url: string): Promise<PDFValidationResult> => {
    ...
    try {
      ...
      if (fileSize && fileSize > 50 * 1024 * 1024) {
        const result: PDFValidationResult = {
          isValid: false,
          error: {
            message: 'PDF file is too large (>50MB). Processing may be slow or fail.',
            type: 'size'
          },
          fileSize,
      ...
      const result: PDFValidationResult = {
        isValid: false,
        error: {
          message: 'Unable to access PDF. Please check the URL and try again.',
          type: 'network'
        }
      };
      setLastValidationResult(result);
